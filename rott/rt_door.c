...
static void (*touchactions[NUMTOUCHPLATEACTIONS])(long) = {ActivatePushWall, ActivateMoveWall, LinkedOpenDoor,
                                                           LinkedCloseDoor,  EnableObject,     DisableObject,
                                                           ActivateLight,    DeactivateLight};

#if ((DEVELOPMENT == 1))
#if ((LOADSAVETEST == 1))
static char *touchstrings[NUMTOUCHPLATEACTIONS] = {"ActivatePushWall", "ActivateMoveWall", "LinkedOpenDoor",
                                                   "LinkedCloseDoor",  "EnableObject",     "DisableObject",
                                                   "ActivateLight",    "DeactivateLight"};
#endif
#endif
...
static char *elevstring[NUMELEVATORACTIONS] = {"ready at source", "ready at destination", "moving to source",
                                               "moving to destination", "doorclosing"};
#endif
#endif
...
#if ((DEVELOPMENT == 1))
#if (LOADSAVETEST == 1)
            DebugOut("action node %d: tictime: %d, ticcount: %d ", k, dummy.tictime, dummy.ticcount);
            if (dummy.actionindex == -1)
                DebugOut("action: -1,");
            else
                DebugOut("action: %13s,", touchstrings[dummy.actionindex]);

            if (dummy.swapactionindex == -1)
                DebugOut("swapaction: -1,");
            else
                DebugOut("swapaction: %13s,", touchstrings[dummy.swapactionindex]);

            if (dummy.whichobj & FL_TACT)
                DebugOut("whichobj (actor): %4x\n", (dummy.whichobj & ~FL_TACT));
            else
                DebugOut("whichobj (nonactor): %4x\n", dummy.whichobj);
#endif
#endif
...
void LoadTouchPlates(byte *buffer, int size)
{
    ...
    for (i = 0; i < lasttouch; i++) {
        ...
        for (k = 0, temp = touchplate[i]; temp; k++, temp = temp->nextaction) {
            DebugOut("action node %d: tictime: %d, ticcount: %d ", k, temp->tictime, temp->ticcount);
            if (!temp->action)
                DebugOut("action: NULL,");
            else
                DebugOut("action: %13s,", touchstrings[GetIndexForAction(temp->action)]);

            if (!temp->swapaction)
                DebugOut("swapaction: NULL,");
            else
                DebugOut("swapaction: %13s,", touchstrings[GetIndexForAction(temp->swapaction)]);

            DebugOut("whichobj: %4x\n", (int)temp->whichobj);
        }
    }
#endif
#endif
...
void Link_To_Touchplate(word touchlocx, word touchlocy, void (*maction)(long), void (*swapaction)(long), long wobj,
                        int delaytime)
{
    touchplatetype *temp;
    int index;
...
void SpawnDoor(int tilex, int tiley, int lock, int texture)
{
    ...
    if ((MAPSPOT(tilex, tiley, 1) >= 29) && (MAPSPOT(tilex, tiley, 1) <= 32)) {
...
void OperateElevatorSwitch(objtype *ob, int elevnum, int checkx, int checky)
{
    ...
    if ((eptr->state == ev_mts) || (eptr->state == ev_mtd)) {
#if (DEVELOPMENT == 1)
#if (ELEVATORTEST == 1)
