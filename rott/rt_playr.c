...
void PollVirtualReality(void)
{
    ...
    union REGS inregs;
    union REGS outregs;
    short int mousexmove, mouseymove;
    word vr_buttons;
...
void PollAssassin(void)
{
    ...
    RampType ramp[MAXRAMPS] = {
        {0, 280000}, {4, 380000}, {10, 480000}, {25, 680000},
        //                              {25,( (1<<26)/80  )}
    };
    ...
    controlbuf[0] += -(FixedMul(SWIFTStatus.pitch, viewcos)) + FixedMul(-SWIFTStatus.roll, costable[strafeAngle]);

    controlbuf[1] += FixedMul(SWIFTStatus.pitch, viewsin) - FixedMul(-SWIFTStatus.roll, sintable[strafeAngle]);
