...
extern void REGARGS c2p1x1_8_c5_bm_040(REG(d0, UWORD chunky_x), REG(d1, UWORD chunky_y), REG(d2, UWORD offset_x),
                                       REG(d3, UWORD offset_y), REG(a0, UBYTE *chunky_buffer),
                                       REG(a1, struct BitMap *bitmap));

extern void REGARGS c2p1x1_6_c5_bm_040(REG(d0, UWORD chunky_x), REG(d1, UWORD chunky_y), REG(d2, UWORD offset_x),
                                       REG(d3, UWORD offset_y), REG(a0, UBYTE *chunky_buffer),
                                       REG(a1, struct BitMap *bitmap));

extern void REGARGS c2p1x1_8_c5_bm(REG(d0, UWORD chunky_x), REG(d1, UWORD chunky_y), REG(d2, UWORD offset_x),
                                   REG(d3, UWORD offset_y), REG(a0, UBYTE *chunky_buffer),
                                   REG(a1, struct BitMap *bitmap));

extern void REGARGS c2p1x1_6_c5_bm(REG(d0, UWORD chunky_x), REG(d1, UWORD chunky_y), REG(d2, UWORD offset_x),
                                   REG(d3, UWORD offset_y), REG(a0, UBYTE *chunky_buffer),
                                   REG(a1, struct BitMap *bitmap));
...
void I_SetPalette(byte *palette)
{
    ...
    case VideoModeINDIVISION:
        ...
        if (playstate != ex_stillplaying) {
            for (i = 0; i < 128; i++)
                video_colourtable[ptranslate[i]] = ((((UBYTE)(gammatable[palette[6 * i + 3]]) & 0xf8) >> 3) << 2 |
...
void GraphicsMode(void)
{
    ...
    if (firsttime) {
        ...
        if (CheckParm("EHB")) {
            ...
            for (i = 0; i < (3); i++) {
                if ((video_raster[i] = (PLANEPTR)AllocRaster(320, video_depth * 200)) == NULL)
            ...
            if ((_hardwareScreen =
                     OpenScreenTags(NULL, SA_Type, CUSTOMSCREEN | CUSTOMBITMAP, SA_DisplayID, 0x00000000, SA_DClip,
                                    (ULONG)&rect, SA_Width, 320, SA_Height, 200, SA_Depth, video_depth, SA_Draggable,
                                    FALSE, SA_AutoScroll, FALSE, SA_Exclusive, TRUE, SA_Quiet, TRUE, SA_BitMap,
                                    (Tag)&video_bitmap[0], /* custom bitmap, contiguous planes */
                                    TAG_DONE, 0)) == NULL) {
            ...
            if (CyberGfxBase != NULL) {
                modeId = BestCModeIDTags(CYBRBIDTG_NominalWidth, 320, CYBRBIDTG_NominalHeight, 200, CYBRBIDTG_Depth,
                                         video_depth, TAG_DONE);
            }
        } else {
            ...
            modeId = BestModeID(BIDTAG_NominalWidth, 320, BIDTAG_NominalHeight, 200, BIDTAG_DesiredWidth, 320,
                                BIDTAG_DesiredHeight, 200, BIDTAG_Depth, video_depth, BIDTAG_MonitorID,
                                CheckParm("NTSC") ? NTSC_MONITOR_ID : PAL_MONITOR_ID, TAG_END);
        }
        ...
        if (vidMode != VideoModeINDIVISION) {
            ...
            _hardwareScreen =
                OpenScreenTags(NULL, SA_Depth, video_depth, SA_DisplayID, modeId, SA_Width, 320, SA_Height, 200,
                               SA_Type, CUSTOMSCREEN, SA_Overscan, OSCAN_TEXT, SA_Quiet, TRUE, SA_ShowTitle, FALSE,
                               SA_Draggable, FALSE, SA_Exclusive, TRUE, SA_AutoScroll, FALSE, TAG_END);
        }
        ...
        _hardwareWindow = OpenWindowTags(
            NULL, WA_Left, 0, WA_Top, 0, WA_Width, 320, WA_Height, 200, WA_Title, (Tag)NULL, SA_AutoScroll, FALSE,
            WA_CustomScreen, (ULONG)_hardwareScreen, WA_Backdrop, TRUE, WA_Borderless, TRUE, WA_DragBar, FALSE,
            WA_Activate, TRUE, WA_SimpleRefresh, TRUE, WA_NoCareRefresh, TRUE, WA_ReportMouse, TRUE, WA_RMBTrap, TRUE,
            WA_IDCMP, IDCMP_RAWKEY | IDCMP_MOUSEMOVE | IDCMP_DELTAMOVE | IDCMP_MOUSEBUTTONS, TAG_END);
...
void I_FinishUpdate(void)
{
    ...
    if (vidMode == VideoModeAGA) {
        ...
        video_bitmap_handle =
            LockBitMapTags(_hardwareScreen->ViewPort.RasInfo->BitMap, LBMI_BASEADDRESS, (Tag)&base_address, TAG_DONE);
