...
static void decode_color(Uchar *data, Flic *flic, Screen *s, ColorOut *output)
 ...
 * Seek to first frame. */
{
    ...
    if ((err = SetupFlicAccess(flic)) >= Success) {
        if ((err = CopyNextFlicBlock(flic, (Uchar *)&flic->head, sizeof(flic->head))) >= Success) {
...
static ErrCode decode_frame(Flic *flic, FrameHead *frame, Uchar *data, Screen *s)
...
static char *err_strings[] = {
    "Unspecified error",     "Not enough memory",      "Not a flic file",      "Bad frame in flic", NULL, NULL,
    "Couldn't open display", "Couldn't open keyboard", "User canceled action",
};
