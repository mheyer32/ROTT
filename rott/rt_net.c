...
byte *demoptr, *lastdemoptr, *demobuffer = NULL;
    ...
    if (modemgame == true) {
        ...
        if (networkgame == true) {
            ...
            if (IsServer == true) {
                ...
                for (j = 0; j < MAXCMDS; j++) {
                    int size;
                    size = ((numplayers * GetTypeSize(COM_TEXT)) + GetTypeSize(COM_SOUNDANDDELTA) +
                            sizeof(COM_ServerHeaderType) - sizeof(byte));
                    ServerCommand(j) = SafeMalloc(size);
                    memset(ServerCommand(j), COM_DELTANULL, size);
                }
                for (i = 1; i <= numplayers; i++) {
                    CommandState[i] = (CommandStatusType *)SafeMalloc(sizeof(CommandStatusType));
                }
                for (i = 0; i < numplayers; i++) {
...
void StartupClientControls(void)
{
    ...
    for (j = 0; j < numplayers; j++) {
        memset(PLAYERSTATE[j].buttonheld, 0, sizeof(PLAYERSTATE[j].buttonheld));
        memset(PLAYERSTATE[j].buttonstate, 0, sizeof(PLAYERSTATE[j].buttonstate));
    }
    ...
    if (IsServer == true) {
        ...
        UpdateServer = true;
        size = ((numplayers * GetTypeSize(COM_TEXT)) + GetTypeSize(COM_SOUNDANDDELTA) + sizeof(COM_ServerHeaderType) -
                sizeof(byte));
...
#if (DEVELOPMENT == 1)
//   ComError("StartupClientControls: GetTicCount()=%ld oldtime=%ld
//   controlupdatetime=%ld\n",GetTicCount(),oldtime,controlupdatetime);
#endif
...
void UpdateClientControls(void)
{
   ...
   if (delta>largesttime)
    ...
    if (standalone == false) {
        ...
        if ((networkgame == false) && (ServerCommandStatus(oldpolltime) == cs_fixing)) {
        ...
        while (time >= controlupdatetime) {
            ...
            if ((memcmp(&controlbuf[0], &oldcontrolbuf[0], sizeof(controlbuf)) != 0) || (buttonbits != oldbuttonbits)) {
                controlschanged = true;
            ...
            if ((controlschanged == false) && (soundready == false)) {
                ...
                if (soundready == true) {
                    ...
                    status = SD_GetSoundData(&(sndpkt->data[0]), COM_SOUND_BUFFERSIZE);
                    switch (status) {
                    case rs_nodata:
                        Delta->type = COM_DELTA;
                        break;
...
void ProcessRemoteRidicule(void *pkt)
{
    ...
    if ((who == consoleplayer) || (who == MSG_DIRECTED_TO_ALL) ||
        ((who == MSG_DIRECTED_TO_TEAM) && (BATTLE_Team[from] == BATTLE_Team[consoleplayer]))) {
        strcpy(name, "( RR from ");
...
void SendPacket(void *pkt, int dest)
{
    ...
    else if ((IsServer == false) && (dest != server) &&
             (standalone == false))  // We shouldn't be sending as client to anyone else
...
void ResendServerPackets(int time, int dest, int numpackets)
{
    ...
    if (serverpkt->time != time) {
        Error("SERVER: Could not find packet to resend\ntime=%d packettime=%d serverupdatetime=%d\n", time,
              serverpkt->time, serverupdatetime);
    } else {
        byte *tempbuf;
        byte *tempptr;
        byte *tempstart;
...
void ProcessSyncCheckPacket(void *pkt, int src)
{
    ...
    if (sync->randomindex != PlayerSync[0].randomindex) {
        Error(
            "Player %d is unsynced localindex=%d remoteindex=%d\n"
            "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
            src, PlayerSync[0].randomindex, sync->randomindex, PlayerSync[src].x, PlayerSync[src].y,
            PlayerSync[src].angle, PlayerSync[src].z, PLAYERSTATE[src].codename);
    }
    if (sync->x != PlayerSync[src].x) {
        Error(
            "Player %d is unsynced local x=%d remote x=%d\n"
            "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
            src, PlayerSync[src].x, sync->x, PlayerSync[src].x, PlayerSync[src].y, PlayerSync[src].angle,
            PlayerSync[src].z, PLAYERSTATE[src].codename);
    }
    if (sync->y != PlayerSync[src].y) {
        Error(
            "Player %d is unsynced local y=%d remote y=%d\n"
            "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
            src, PlayerSync[src].y, sync->y, PlayerSync[src].x, PlayerSync[src].y, PlayerSync[src].angle,
            PlayerSync[src].z, PLAYERSTATE[src].codename);
    }
    if (sync->z != PlayerSync[src].z) {
        Error(
            "Player %d is unsynced local z=%d remote z=%d\n"
            "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
            src, PlayerSync[src].z, sync->z, PlayerSync[src].x, PlayerSync[src].y, PlayerSync[src].angle,
            PlayerSync[src].z, PLAYERSTATE[src].codename);
    }
    if (sync->angle != PlayerSync[src].angle) {
        Error(
            "Player %d is unsynced local angle=%d remote angle=%d\n"
            "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
            src, PlayerSync[src].angle, sync->angle, PlayerSync[src].x, PlayerSync[src].y, PlayerSync[src].angle,
            PlayerSync[src].z, PLAYERSTATE[src].codename);
    }
}
...
void AddSubPacket(void *pkt, int src)
{
    ...
    memcpy(ClientTimeCommand(src, packet->time), pkt, GetPacketSize(packet));
}
...
void AddPacket(void *pkt, int src)
{
    ...
    if (!((src == server) && (standalone == false) && (IsServer == true))) {
        ...
        if (packet->time != LastCommandTime[src]) {
            ...
            if (((networkgame == false) && (ServerCommandStatus(LastCommandTime[src]) != cs_fixing)) ||
                ((networkgame == true) && (ClientCommandStatus(src, LastCommandTime[src]) != cs_fixing))) {
                RequestPacket(LastCommandTime[src], src, numpackets);

                ComError("AddPacket: Request packet time=%d lct=%d numpackets=%d\n", packet->time, LastCommandTime[src],
                         numpackets);
            }
...
{
    if (((COM_ServerHeaderType *)ServerCommand(CommandAddress(time)))->time == time)
        return true;
    else {
        return false;
    }
...
void ProcessPlayerCommand(int player)
{
    ...
    if (cmd->type == COM_DELTA) {
        ...
        if ((who == consoleplayer) || (who == MSG_DIRECTED_TO_ALL) ||
            ((who == MSG_DIRECTED_TO_TEAM) && (BATTLE_Team[player] == BATTLE_Team[consoleplayer]))) {
            char string[50];
...
void UpdatePlayerObj(int player)
{
    ...
    if (demoplayback || demorecord) {
        SoftError("  dmx=%4x dmy=%4x da=%4x time=%5d\n", pstate->dmomx, pstate->dmomy, pstate->angle >> 11,
                  oldpolltime);
    }
#if 0
#if (DEVELOPMENT == 1)
...
void SendPlayerDescription(void)
{
    ...
    strcpy(&(desc->playerdescription.codename[0]), &locplayerstate->codename[0]);
...
void SetGameDescription(void *pkt)
{
    ...
    for (i = 0; i < numplayers; i++) {
        ...
        strcpy(&(PLAYERSTATE[i].codename[0]), &(desc->players[i].codename[0]));
    }
    AssignTeams();
}
...
void SetPlayerDescription(void *pkt)
{
    ...
    strcpy(&(PLAYERSTATE[desc->player].codename[0]), &(desc->playerdescription.codename[0]));
