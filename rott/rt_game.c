...
void GM_MemToScreen(byte *source, int width, int height, int x, int y)
{
    ...
    for (plane = 0; plane < 4; plane++) {
        ...
        screen3 = dest3;
        for (y = 0; y < height;
             y++, screen1 += linewidth, screen2 += linewidth, screen3 += linewidth, source += width) {
#ifdef DOS
...
void CheckHighScore(long score, word other, boolean INMENU)
{
    ...
    if (n != -1) {
        PrintY = 25 + (16 * n);
        PrintX = 3 * 8;
        US_LineInput(PrintX, PrintY, Scores[n].name, NULL, true, 10, 98, 0);
    } else {
        IN_ClearKeysDown();
        if (INMENU) {
            while (!IN_CheckAck()) {
...
void BattleLevelCompleted(int localplayer)
{
    ...
    while (1) {
        if (Screen != LastScreen) {
            ...
            sprintf(text,
                    "Page %d of 3.  Use arrows to switch stats.  "
                    "Press Esc to quit.",
                    Screen);
...
boolean SaveTheGame(int num, gamestorage_t *game)
{
...
#if PLATFORM_DOS
    {
        ...
        avail = (int)dfree.avail_clusters * dfree.bytes_per_sector * dfree.sectors_per_cluster;
        avail -= 8192;
