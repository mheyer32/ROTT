...
void FX_SetReverseStereo(int setting) {}  // FX_SetReverseStereo
...
static int setupVocPlayback(char *ptr, int size, int priority, unsigned long callbackval, int *chan, Mix_Chunk **chunk)
{
...
int FX_PlayLoopedVOC(char *ptr, long loopstart, long loopend, int pitchoffset, int vol, int left, int right,
                     int priority, unsigned long callbackval)
{
    ...
    if ((loopstart >= 0) && ((unsigned int)loopstart < totalsamples)) {
        if (loopend < 0)
            loopend = 0;
        if ((unsigned int)loopend > totalsamples)
        ...
        if (loopstart > 0) {
            ...
            memcpy(chunk->abuf, ((unsigned char *)chunk->abuf) + loopstart, chunk->alen - loopstart);
...
// ROTT Special - SBF
int FX_PlayVOC3D_ROTT(char *ptr, int size, int pitchoffset, int angle, int distance, int priority,
...
int FX_PlayLoopedWAV(char *ptr, long loopstart, long loopend, int pitchoffset, int vol, int left, int right,
                     int priority, unsigned long callbackval)
{
    return (FX_PlayLoopedVOC(ptr, loopstart, loopend, pitchoffset, vol, left, right, priority, callbackval));
}  // FX_PlayLoopedWAV
...
// ROTT Special - SBF
int FX_PlayWAV3D_ROTT(char *ptr, int size, int pitchoffset, int angle, int distance, int priority,
                      unsigned long callbackval)
{
    return (FX_PlayVOC3D_ROTT(ptr, size, pitchoffset, angle, distance, priority, callbackval));
}  // FX_PlayWAV3D_ROTT

int FX_PlayRaw(char *ptr, unsigned long length, unsigned rate, int pitchoffset, int vol, int left, int right,
               int priority, unsigned long callbackval)
{
...
int FX_PlayLoopedRaw(char *ptr, unsigned long length, char *loopstart, char *loopend, unsigned rate, int pitchoffset,
...
int FX_StartDemandFeedPlayback(void (*function)(char **ptr, unsigned long *length), int rate, int pitchoffset, int vol,
