...
#ifdef DOS
int musicnums[11] = {-1,          UltraSound, SoundBlaster, SoundMan16, ProAudioSpectrum, Awe32, SoundScape,
                     WaveBlaster, GenMidi,    SoundCanvas,  Adlib};

int fxnums[11] = {-1,    UltraSound,  SoundBlaster,     SoundMan16, ProAudioSpectrum, Awe32, SoundScape,
                  Adlib, SoundSource, TandySoundSource, PC};
#else
...
int SD_SetupFXCard(int *numvoices, int *numbits, int *numchannels)
{
    ...
    if ((card == SoundBlaster) || (card == Awe32)) {
        ...
        status = FX_SetupSoundBlaster(SBSettings, numvoices, numbits, numchannels);
    } else {
#endif
...
int SD_PlayIt(int sndnum, int angle, int distance, int pitch)
{
    ...
    voice = FX_PlayWAV3D(snd, pitch, angle, distance, sounds[sndnum].priority, (unsigned long)sndnum);
...
int MU_Startup(boolean bombonerror)
{
    ...
    if ((card == SoundBlaster) || (card == Awe32) || (card == WaveBlaster)) {
        if (SD_Started == false) {
            ...
            FX_SetupSoundBlaster(SBSettings, &numvoices, &numbits, &numchannels);
        }
    }
