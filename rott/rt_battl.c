...
static int BATTLE_CheckKills(battle_event reason, int player);
...

{
    ...
    if (BattleMode != battle_StandAloneGame) {
        ...
        if (gamestate.Product == ROTT_SHAREWARE) {
            ...
            default:
                Error(
                    "Shareware version can only play Normal, Collector, "
...

{
    ...
    case battle_refresh:
        Timer++;
        if ((TimeLimit != INFINITE) && (Timer > TimeLimit)) {
            RoundOver = true;
    ...
    default:
#if (BATTLECHECK == 1)
        Error("BATTLE_CheckGameStatus called with a reason of %d.", reason);
#else
        return (battle_no_event);
#endif
        break;
...

{
...
#if (BATTLECHECK == 1)
        Error(
            "BATTLE_PlayerKilledPlayer : Player "
            "killed self with illegal reason of %d.",
            reason);
#else
        return (battle_no_event);
#endif
    ...
    if ((consoleplayer == victim) && (reason == battle_kill_by_crushing)) {
    ...
    if (BattleMode == battle_ScoreMore) {
        ...
        if (reason == battle_player_tagged) {
            ...
            Error(
                "BATTLE_PlayerKilledPlayer - reason %d : "
...
#if (BATTLECHECK == 1)
            Error(
                "BATTLE_PlayerKilledPlayer called with a "
                "reason of %d in Hunter.",
                reason);
#else
            ;
#endif
