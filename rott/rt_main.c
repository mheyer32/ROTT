...
static char const *const flags[] = {"NOSOUND", "EHB", "CGX", "AGA", "NTSC", "INDIVISION", "LOMEM"};
...
void CheckCommandLineParameters(void)
{
    static char const *const PStrings[] = {
        "TEDLEVEL", "NOWAIT",    "NOSOUND",      "NOW",    "TRANSPORT", "DOPEFISH",   "SCREENSHOTS", "MONO",
        "MAPSTATS", "TILESTATS", "VER",          "net",    "PAUSE",     "SOUNDSETUP", "WARP",        "IS8250",
        "ENABLEVR", "TIMELIMIT", "MAXTIMELIMIT", "NOECHO", "DEMOEXIT",  "QUIET",      NULL};
    int i, n;
...
#endif
                    if ((!LastScan) && (!IN_GetMouseButtons()) && (lowmemory == 0) && (GameLevels.avail == false)) {
                        if (demonumber == -1)
        ...
        case ex_resetgame:
            ...
            if (modemgame == true) {
                ...
                if (gamestate.Version < ROTTVERSION) {
                    Error(
                        "This version of Rise of the Triad (%d.%d) is incompatible with\n"
                        "version %d.%d.",
...
void WritePCX(char *file, byte *source)
{
    ...
    for (y = 0; y < iGLOBAL_SCREENHEIGHT; ++y)  // for each line in band
        if (PutBytes(((unsigned char *)(source + (y * iGLOBAL_SCREENWIDTH))), pcxHDR.bytesperline))
