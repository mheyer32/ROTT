...
#define ServerCommand(command) (ServerCmds->Commands[(command)])

#define CommandAddress(time) (((time)-controlupdatestartedtime) & (MAXCMDS - 1))

#define NextLocalCommand() (LocalCommand(CommandAddress(controlupdatetime)))
...
#define PacketAddress(time) (time)
