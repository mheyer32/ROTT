...
void MiscPreCache(void)
{
    ...
    if (gamestate.violence >= vl_high) {  // cache in all gibs
        if (DoPanicMapping() == true) {
    ...
    */
    ...
    if (loadedgame == false) {
        ...
        for (i = 1; i < cacheindex; i++) {
            ...
            while (lastmem <= currentmem) {  // SetTextMode (  );
...
int GetNextMap(int tilex, int tiley)
{
    ...
    if (((next & 0xff00) != 0xe200) && ((next & 0xff00) != 0xe400)) {
        // Should this be DEVELOPMENT only?
        Error(
            "GetNextMap : Illegal destination map %xh at exit "
            "tile on map %d.",
            next, gamestate.mapon);
    }
...
// Error("Okay");
    ...
    DebugOut("------            ----          --------\n");
    for (i = 0; i < numplayers; i++)
        DebugOut("  %d             %d            %3d,%3d\n", i, PLAYERSTATE[i].team, PLAYER[i]->tilex,
                 PLAYER[i]->tiley);
...
{
    int i, j, minutes, seconds, starti;
...
void LinkActor(objtype *ob, int tilex, int tiley, void (*action)(long), void (*swapaction)(long))
{
    word touchx, touchy;
    int clockx, clocky;
    int clocklinked, k;
            ...
            case 43:
                if (MAPSPOT(i, j, 2)) {
                    ...
                    if (tswitch && (tswitch->which == WALL)) {
                        ...
                        if (touchindices[touchx][touchy]) {
                            Link_To_Touchplate(touchx, touchy, DeactivateLight, ActivateLight, (long)(sprites[i][j]),
                                               0);
                            sprites[i][j]->linked_to = touchindices[touchx][touchy] - 1;
                        } else
                            Error("tried to link a light at x=%d y=%d to a non-existent touchplate", i, j);
...
void InsaneDump(void)
{
    ...
    for (i = 0; i < 1000; i++)
        if (i < 90) {
            MapDebug("%4d      %4d       %s", i, tally[i], W_GetNameForNum(GetLumpForTile(i)));
            MapDebug("     ");
